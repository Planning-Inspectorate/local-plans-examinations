...
export class DocumentUtil {
	...
	protected static async syncDocuments(
		...
		for (const document of existingDocuments) {
			const fileId = this.getDocumentUploadedFileId(document);
			if (fileId) {
			...
			for (const document of existingDocuments) {
				...
				if (fileId && uploadedFileIds.has(fileId)) {
					if (document.isDeleted || document.latestDocumentVersion?.isDeleted) {
						await this.restoreDocument(tx, document);
					}
					continue;
				}

				if (!document.isDeleted) {
					await this.softDeleteDocument(tx, document.guid);
				}
			}
	...
	protected static mapDocumentToUploadedFile(document: DocumentRow): UploadedFile | undefined {
	...
	protected static getDocumentUploadedFileId(document: DocumentRow): string | undefined {
	...
	protected static async softDeleteDocument(tx: TransactionClient, documentGuid: string) {
		await tx.document.update({
			where: {
				guid: documentGuid
			},
	...
	protected static async restoreDocument(tx: TransactionClient, document: DocumentRow) {
		await tx.document.update({
			where: {
				guid: document.guid
			},
