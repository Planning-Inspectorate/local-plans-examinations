locals {
  ...
  secrets = [
    "localplans-client-secret",
    "localplans-gov-notify-api-key",
  ...
  tech_emails = [for rec in azurerm_monitor_action_group.local_plans_tech.email_receiver : rec.email_address]
  action_group_ids = {
    tech            = azurerm_monitor_action_group.local_plans_tech.id
    service_manager = azurerm_monitor_action_group.local_plans_service_manager.id
