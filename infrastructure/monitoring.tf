...
resource "azurerm_monitor_action_group" "local_plans_tech" {
  name                = "pins-ag-local_plans-tech-${var.environment}"
  resource_group_name = azurerm_resource_group.primary.name
  short_name          = "LocalPlans" # needs to be under 12 characters
  tags                = local.tags
...
resource "azurerm_monitor_action_group" "local_plans_service_manager" {
  name                = "pins-ag-local_plans-service-manager-${var.environment}"
  resource_group_name = azurerm_resource_group.primary.name
  short_name          = "LocalPlans" # needs to be under 12 characters
  tags                = local.tags
