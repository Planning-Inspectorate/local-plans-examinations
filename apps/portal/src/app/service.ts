...
export function derivePlanProgress(caseRecord: PortalCase): Pick<Plan, 'stage' | 'status'> {
	...
	if (caseRecord.gateway2Info?.reportIssuedDate || getGateway2ReportUploadedDate(caseRecord)) {
		return {
...
function mapCaseToPlan(caseRecord: PortalCase): Plan | null {
	...
	const gateway2ReportUploadedDate = getGateway2ReportUploadedDate(caseRecord);
	const gateway2Date =
		caseRecord.gateway2Info?.reportIssuedDate ??
		gateway2ReportUploadedDate ??
