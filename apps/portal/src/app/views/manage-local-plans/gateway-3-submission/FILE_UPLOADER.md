# Gateway 3 File Uploader

This document explains how the Gateway 3 submission file uploader works.

## Files Involved

| File                                                         | Responsibility                                                                                                             |
| ------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `questions.ts`                                               | Defines the Gateway 3 file upload questions, their URLs, field names, copy, validators and upload limits.                  |
| `journey.ts`                                                 | Adds the upload questions to the `Procedural documents` / `Consultation documents` section of the Gateway 3 journey.       |
...
| `packages/database/src/seed/static-data/document-set.ts`     | Seeds the Gateway 3 document sets used by the uploader.                                                                    |
...
## Current Gateway 3 Upload Questions

The configured questions are exported from `questions.ts` as `gateway3FileUploadQuestions`.
Each question must have a unique `fieldName` and `url`.

| Question                    | Field name                          | URL / folder name                       | File count            |
| --------------------------- | ----------------------------------- | --------------------------------------- | --------------------- |
| Gateway 3 cover letter      | `gateway3CoverLetter`               | `gateway-3-cover-letter`                | 1                     |
| Local plan timetable        | `gateway3LocalPlanTimetable`        | `gateway-3-local-plan-timetable`        | 3                     |
| Project initiation document | `gateway3ProjectInitiationDocument` | `gateway-3-project-initiation-document` | Unlimited in practice |

The `url` is important because it is used for three things:

1. The route path, for example `/procedural/gateway-3-local-plan-timetable`.
...
Gateway 3 has shared routes for all file upload questions. The current question is identified by the `:question` route parameter.

```text
/:planReference/gateway-3-submission/:section/:question
...
```mermaid
flowchart TD
    A[User opens Gateway 3 submission] --> B[index.ts loads journey]
    B --> C[questions.ts provides Gateway 3 upload configs]
...
The route is shared, so `index.ts` derives a few route-friendly structures from `gateway3FileUploadQuestions`.

```ts
const gateway3FileUploadQuestionConfigs = Object.values(gateway3FileUploadQuestions);
...
| Value                               | Used for                                                                                          |
| ----------------------------------- | ------------------------------------------------------------------------------------------------- |
| `gateway3FileUploadQuestionConfigs` | Loading persisted uploads for every Gateway 3 upload question when the case page opens.           |
| `gateway3FileUploadQuestionUrls`    | Telling `fileUploaderQuestionMiddleware` which question URLs are file upload pages.               |
| `gateway3FileUploadQuestionsByUrl`  | Resolving `req.params.question` to the correct upload question config during upload/delete POSTs. |

The important behaviour is:

```text
req.params.question = "gateway-3-local-plan-timetable"
          |
          v
gateway3FileUploadQuestionsByUrl.get("gateway-3-local-plan-timetable")
          |
          v
gateway3LocalPlanTimetableQuestion config
```
...
sequenceDiagram
    ...
    Router->>Docs: saveGateway3Documents(question.url, uploadedFiles)
...
flowchart TD
    ...
    D --> E[index.ts calls saveGateway3Documents]
...
```text
LPE-TEST-001:gateway3LocalPlanTimetable
```

The Gateway 3 journey answer is also kept in sync under the journey ID:

```text
session.forms[planReference]["gateway-3-application"][fieldName]
```
...
```text
gateway-3-local-plan-timetable
  -> DocumentSet.folderName = gateway-3-local-plan-timetable
  -> DocumentSet.id = g3-timetable
  -> Document.documentSetId = g3-timetable
```

## Document Sync Behaviour

`saveGateway3Documents` accepts the full current uploader file list for one question.
...
The route layer should not need a new route. The shared dispatcher will pick it up from `gateway3FileUploadQuestions`.
