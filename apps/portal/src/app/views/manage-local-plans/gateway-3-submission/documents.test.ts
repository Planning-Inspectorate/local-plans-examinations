...
import { getDocumentSetIdsByFolderName, loadGateway3Documents, saveGateway3Documents } from './documents.ts';

const GATEWAY_3_COVER_LETTER_DOCUMENT_SET_ID = DOCUMENT_SET_ID.G3_COVER_LETTER;
const TEST_DOCUMENT_SET_FOLDER_NAME = DOCUMENT_SET_FOLDER_NAME.G3_COVER_LETTER;

describe('loadGateway3Documents', () => {
	it('loads documents as uploaded files', async () => {
			...
			existingDocuments: [
				{
					guid: 'document-1',
					name: 'stored-file',
					documentSetId: GATEWAY_3_COVER_LETTER_DOCUMENT_SET_ID,
					isDeleted: false,
					latestDocumentVersion: {
						...
						blobStoragePath: 'gateway-3/cover-letter.pdf',
		...
		const files = await loadGateway3Documents(
		...
		assert.deepEqual(files, [
			{
				...
				path: 'gateway-3/cover-letter.pdf',
...
describe('getDocumentSetIdsByFolderName', () => {
	it('loads document set IDs for folder names in one query', async () => {
		const service = createMockService({
			documentSets: [
				{ id: GATEWAY_3_COVER_LETTER_DOCUMENT_SET_ID, folderName: TEST_DOCUMENT_SET_FOLDER_NAME },
				{ id: DOCUMENT_SET_ID.G3_LOCAL_PLAN_TIMETABLE, folderName: DOCUMENT_SET_FOLDER_NAME.G3_LOCAL_PLAN_TIMETABLE }
			]
		});

		const documentSetIdsByFolderName = await getDocumentSetIdsByFolderName(service as unknown as PortalService, [
			TEST_DOCUMENT_SET_FOLDER_NAME,
			DOCUMENT_SET_FOLDER_NAME.G3_LOCAL_PLAN_TIMETABLE,
			TEST_DOCUMENT_SET_FOLDER_NAME
		]);
		...
		assert.deepEqual(service.db.documentSet.findMany.mock.calls[0].arguments[0], {
			where: {
				folderName: {
					in: [TEST_DOCUMENT_SET_FOLDER_NAME, DOCUMENT_SET_FOLDER_NAME.G3_LOCAL_PLAN_TIMETABLE]
				}
			},
			select: {
				id: true,
		...
		assert.equal(documentSetIdsByFolderName.get(TEST_DOCUMENT_SET_FOLDER_NAME), GATEWAY_3_COVER_LETTER_DOCUMENT_SET_ID);
		assert.equal(
			documentSetIdsByFolderName.get(DOCUMENT_SET_FOLDER_NAME.G3_LOCAL_PLAN_TIMETABLE),
			DOCUMENT_SET_ID.G3_LOCAL_PLAN_TIMETABLE
		);
	});

	it('rejects when any requested document set is missing', async () => {
		const service = createMockService({
			documentSets: [{ id: GATEWAY_3_COVER_LETTER_DOCUMENT_SET_ID, folderName: TEST_DOCUMENT_SET_FOLDER_NAME }]
		});
		...
		await assert.rejects(
			...
			/Missing document set reference data for "gateway-3-local-plan-timetable"/
		);
	});
});
...
describe('saveGateway3Documents', () => {
	it('creates a document and version for a new uploaded file', async () => {
		...
		assert.equal(documentCreateData.documentSetId, GATEWAY_3_COVER_LETTER_DOCUMENT_SET_ID);
	...
	it('rejects save when the document set reference data is missing', async () => {
		...
		await assert.rejects(
			...
			/Missing document set reference data for "gateway-3-cover-letter"/
		);
	...
	it('requires a loaded case on the request', async () => {
		...
		await assert.rejects(
			...
			/Cannot save Gateway 3 documents without a loaded case/
		);
...
function createMockService({
	documentSet = { id: GATEWAY_3_COVER_LETTER_DOCUMENT_SET_ID },
	documentSets = [{ id: GATEWAY_3_COVER_LETTER_DOCUMENT_SET_ID, folderName: TEST_DOCUMENT_SET_FOLDER_NAME }],
	existingDocuments = [],
...
function buildDocumentRow(overrides: Record<string, unknown> = {}) {
	return {
		guid: 'document-1',
		name: 'gateway-3/cover-letter.pdf',
		documentSetId: GATEWAY_3_COVER_LETTER_DOCUMENT_SET_ID,
		isDeleted: false,
		latestDocumentVersion: {
			version: 1,
