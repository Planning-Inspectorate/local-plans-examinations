...
describe('loadGateway3DocumentsByDocumentSetId', () => {
	...
	it('filters out deleted document versions', async () => {
		const service = createMockService({
			existingDocuments: [
				{
					guid: 'document-1',
	...
	it('filters out documents with no latest version', async () => {
		const service = createMockService({
			existingDocuments: [
				{
					guid: 'document-1',
...
describe('getDocumentSetIdsByFolderName', () => {
	it('loads document set IDs for folder names in one query', async () => {
		...
		assert.deepEqual(service.db.documentSet.findMany.mock.calls[0].arguments[0], {
			where: {
				folderName: {
					in: [TEST_DOCUMENT_SET_FOLDER_NAME, 'map-of-policies']
				}
			},
			select: {
				id: true,
		...
		assert.equal(documentSetIdsByFolderName.get(TEST_DOCUMENT_SET_FOLDER_NAME), G3_DOCUMENT_SET_ID);
		assert.equal(documentSetIdsByFolderName.get('map-of-policies'), 'g3-map-policies');
	});
...
describe('saveGateway3Documents', () => {
	it('creates a document and version for a new uploaded file', async () => {
		...
		assert.equal(documentCreateData.documentSetId, G3_DOCUMENT_SET_ID);
...
function buildDocumentRow(overrides: Record<string, unknown> = {}) {
	return {
		guid: 'document-1',
		name: 'gateway-3/proposed-plan.pdf',
		documentSetId: G3_DOCUMENT_SET_ID,
		isDeleted: false,
		latestDocumentVersion: {
			version: 1,
