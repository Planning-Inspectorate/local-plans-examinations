...
// Loads the saved files for this Gateway 3 upload question.
export async function loadGateway3Documents(
	service: PortalService,
	caseId: string,
	documentSetFolderName: string
): Promise<UploadedFile[]> {
	const documentSetId = await getDocumentSetIdByFolderName(service, documentSetFolderName);
	return loadGateway3DocumentsByDocumentSetId(service, caseId, documentSetId);
}

// Loads the saved files for a Gateway 3 upload question with a known document set ID.
...
// Saves the current Gateway 3 upload state.
