...
// Defines the answers saved for the Gateway 3 journey.
export interface Gateway3ApplicationAnswers {
	gateway3CoverLetter: UploadedFile[];
	gateway3LocalPlanTimetable: UploadedFile[];
	gateway3ProjectInitiationDocument: UploadedFile[];
}

// Creates the controller that handles the final Gateway 3 submission.
export function buildSaveController(service: PortalService): RequestHandler {
	return async (req, res) => {
		...
		const answers = journeyResponse.answers as unknown as Gateway3ApplicationAnswers;
		...
		// Clears the Gateway 3 journey answers after submission.
		const planReference = getRoutePlanReference(req);
		if (planReference) {
			clearDataFromSession({ req, journeyId: JOURNEY_ID, reqParam: 'planReference' });
			return res.redirect(
				`/manage-local-plans/${encodeURIComponent(planReference)}/gateway-3-submission/application-complete`
			);
		}
