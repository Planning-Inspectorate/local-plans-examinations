...
// This file wires the Gateway 3 submission journey into Express.
//
...
// - Route setup: registers the Gateway 3 listing, question, upload and delete
...
// The Gateway 3 upload routes are shared by all of the document questions.
...
const gateway3FileUploadQuestionConfigs = Object.values(gateway3FileUploadQuestions) as Gateway3FileUploadQuestion[];
...
// Fast lookup for POST routes such as `/gateway-3-local-plan-timetable/upload-documents`.
...
const GATEWAY_3_SUBMIT_ERROR = 'Add at least one document before submitting';
...
// TODO: This shared Multer middleware uses the largest Gateway 3 question
...
function setGateway3CheckAnswersViewData(req: Request, res: Response, next: NextFunction) {
	setGateway3CheckAnswersViewLocals(req, res);
	next();
}

function setGateway3CheckAnswersViewLocals(req: Request, res: Response) {
	const request = req as Gateway3Request;
	const planReference = getRoutePlanReference(req);
...
function validateGateway3Submission(): RequestHandler {
	return (req, res, next) => {
		const journeyResponse = res.locals.journeyResponse as JourneyResponse | undefined;
		const answers = journeyResponse?.answers ?? {};
		const hasAnsweredQuestion = gateway3FileUploadQuestionConfigs.some((questionConfig) => {
		...
		return buildGateway3CheckAnswersList()(req, res, next);
	};
}
...
// Example format: LP-TEST-001:gateway3CoverLetter.
...
// Keeps a Gateway 3 file upload answer in sync with uploaded files.
...
// Registers the Gateway 3 submission routes.
export function gateway3SubmissionRoutes(service: PortalService): IRouter {
	...
	router.get(
		...
		getJourney,
		setAsEditingFromCya,
		setGateway3CheckAnswersViewData,
		buildGateway3CheckAnswersList()
	);

	router.post('/gateway-3-submission', getJourneyResponse, getJourney, validateGateway3Submission(), saveToDatabase);

	router.use(
		'/:planReference/gateway-3-submission/application-declaration',
		createApplicationDeclarationRoutes(service)
	);

	router.use('/:planReference/gateway-3-submission/application-complete', createApplicationCompleteRoutes());
