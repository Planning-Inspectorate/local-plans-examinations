...
const VIEW_PATH = 'views/manage-local-plans/gateway-3-submission/application-declaration/application-declaration.njk';
...
describe('buildGetDeclarationPage', () => {
	it('renders the declaration page with correct view data', async () => {
		...
		assert.equal(model.backLinkUrl, '/manage-local-plans/PLAN%2F123456/gateway-3-submission');
	});
});
...
describe('buildPostDeclarationPage', () => {
	it('redirects to application-complete when both checkboxes are checked', async () => {
		...
		assert.equal(redirectCalls[0], '/manage-local-plans/PLAN%2F123456/gateway-3-submission/application-complete');
