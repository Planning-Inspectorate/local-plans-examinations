...
import { syncGateway3UploadAnswer } from './index.ts';
...
describe('syncGateway3UploadAnswer', () => {
	it('stores uploaded files in the case-scoped journey answers', () => {
		...
		syncGateway3UploadAnswer(req as unknown as Request, 'gateway3CoverLetter', [uploadedFile]);
	...
	it('removes the case-scoped journey answer when no uploaded files remain', () => {
		const req = {
			...
			session: {
				forms: {
					'LPE-TEST-001': {
						[JOURNEY_ID]: {
							gateway3CoverLetter: [buildUploadedFile({ id: 'file-1' })]
						}
					}
				}
			}
		...
		syncGateway3UploadAnswer(req as unknown as Request, 'gateway3CoverLetter', []);
